function [simGridParam, PV_N, tubeStartArea, tubeStart, tubeEnd, totalTubeLengthInCells, currTubeSectionDiameterCells_SegmentCounter, listenerInfo, sourceInfo] = vt_squareTubeGeneration(simulationType, junctionType, vowel, boundaryInterpolation, gridCellTypes, baffleSwitchFlag, pmlSwitch, pmlLayer, rad, ds)
    ...
    % Calculate the length of the sqaure (in meter) using the tube 
    % cross-sectional area function.

    squareLen_inm = sqrt(tubeSectionArea_inm2);
    
    tubeSquareLenInCells = round(squareLen_inm./ds);
        
    % Cross-sectional area at the tube start
    % I didn't use "tubeSectionArea_inm2" directly because I wanted to use
    % the derived area
    tubeStartArea = squareLen_inm(1)*squareLen_inm(1);
         
    % STEP XX: Choose the best possible odd number from the tubeSquareLenInCells 
    for counter = 1:numSections
        
        % Verify if the tubeMajorAxisLenInCells is odd or not
        if mod(tubeSquareLenInCells(counter), 2) == 0
            
            % Find the difference between the rounded and the actual
            % diameter value = Estimated diameter-Actual diameter
            diff = tubeSquareLenInCells(counter) - ...
                (squareLen_inm(counter)/ds);
            
            if diff>0
                tubeSquareLenInCells(counter) = ...
                    tubeSquareLenInCells(counter)-1;
            else
                tubeSquareLenInCells(counter) = ...
                    tubeSquareLenInCells(counter)+1;
            end    
    ...
    if baffleSwitchFlag == 1
        ...
        domainY = max(tubeSquareLenInCells) + 2; % +2 is tube walls
        domainZ = max(tubeSquareLenInCells) + 2; % +2 is tube walls
    end
    ...
    if junctionType == 1
        ...
        = vt_centricSquareContourGeneration(PV_N, numSections, totalTubeLengthInCells, tubeSquareLenInCells, tubeCummSectionLength, boundaryInterpolation, tubeStart, gridCellTypes, ds);
    else
        [PV_N, currTubeSectionDiameterCells_SegmentCounter]...
        = vt_eccentricSquareContourGeneration(PV_N, numSections, totalTubeLengthInCells, tubeSquareLenInCells, tubeCummSectionLength, boundaryInterpolation, tubeStart, pmlSwitch, pmlLayer, gridCellTypes, ds);
    end
